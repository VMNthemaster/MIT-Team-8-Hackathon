...
year2020Router.post('/exchangerate', async (req, res) => {
    ...
    try {
        if(day < 10){
            allDocuments = await year2020.findOne({Date: `${day}-${month}-${year}`})
            if(!allDocuments){
            allDocuments = await year2020.findOne({Date: `0${day}-${month}-${year}`})
            }
        }
        else{
            allDocuments = await year2020.findOne({Date: `${day}-${month}-${year}`})
