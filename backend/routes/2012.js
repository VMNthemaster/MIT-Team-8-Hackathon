...
year2012Router.post('/halfYearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2012Router.post('/quarterYearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2012Router.post('/monthlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2012Router.post('/exchangerate', async (req, res) => {
    ...
    try {
        if(day < 10){
            allDocuments = await year2012.findOne({Date: `${day}-${month}-${year}`})
            if(!allDocuments){
            allDocuments = await year2012.findOne({Date: `0${day}-${month}-${year}`})
            }
        }
        else{
            allDocuments = await year2012.findOne({Date: `${day}-${month}-${year}`})
