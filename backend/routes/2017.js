...
import year2017 from '../models/2017.js'

const year2017Router = express.Router()

year2017Router.post('/yearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
    const list = [firstCurrency, secondCurrency]
    let allDocuments

    try {
        allDocuments = await year2017.find({})
...
year2017Router.post('/halfYearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2017Router.post('/quarterYearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2017Router.post('/monthlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2017Router.post('/exchangerate', async (req, res) => {
    ...
    try {
        if(day < 10){
            allDocuments = await year2017.findOne({Date: `${day}-${month}-${year}`})
            if(!allDocuments){
            allDocuments = await year2017.findOne({Date: `0${day}-${month}-${year}`})
            }
        }
        else{
            allDocuments = await year2017.findOne({Date: `${day}-${month}-${year}`})
