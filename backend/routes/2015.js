...
import year2015 from '../models/2015.js'

const year2015Router = express.Router()

year2015Router.post('/yearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
    const list = [firstCurrency, secondCurrency]
    let allDocuments

    try {
        allDocuments = await year2015.find({})
...
year2015Router.post('/halfYearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2015Router.post('/quarterYearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2015Router.post('/monthlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2015Router.post('/exchangerate', async (req, res) => {
    ...
    try {
        if(day < 10){
            allDocuments = await year2015.findOne({Date: `${day}-${month}-${year}`})
            if(!allDocuments){
            allDocuments = await year2015.findOne({Date: `0${day}-${month}-${year}`})
            }
        }
        else{
            allDocuments = await year2015.findOne({Date: `${day}-${month}-${year}`})
