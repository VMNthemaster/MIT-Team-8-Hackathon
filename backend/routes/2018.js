...
import year2018 from '../models/2018.js'

const year2018Router = express.Router()

year2018Router.post('/yearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
    const list = [firstCurrency, secondCurrency]
    let allDocuments

    try {
        allDocuments = await year2018.find({})
...
year2018Router.post('/halfYearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2018Router.post('/quarterYearlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2018Router.post('/monthlyData', async (req, res) => {
    const {firstCurrency, secondCurrency} = req.body
...
year2018Router.post('/exchangerate', async (req, res) => {
    ...
    try {
        if(day < 10){
            allDocuments = await year2018.findOne({Date: `${day}-${month}-${year}`})
            if(!allDocuments){
            allDocuments = await year2018.findOne({Date: `0${day}-${month}-${year}`})
            }
        }
        else{
            allDocuments = await year2018.findOne({Date: `${day}-${month}-${year}`})
